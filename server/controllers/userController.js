...
if (!JWT_SECRET) {
  console.error(
    "FATA: ERROR: JWT_SECRET is not defined in environment variables."
  );
  process.exit(1);
}
...
export const createUser = async (req, res) => {
  try {
    ...
    if (error.code === 11000 && error.keyPattern && error.keyPattern.email) {
      ...
      return res
        .status(400)
        .json({ message: "Balidation failed", errors: errors });
    }
  ...
  try {
    ...
    if (user.status !== "active") {
      ...
      switch (user.status) {
        case "pending":
          statusMessage =
            "Your account is pending activation. Please check your email for verification";
          break;
        case "rejected":
          statusMessage =
    ...
    if (!user.password) {
      ...
      return res
        .status(500)
        .json({ message: "Server error: User data corrupted." });
    }
...
export const getAllUsers = async (req, res) => {
  try {
    ...
    res
      .status(404)
      .json({ message: "Users Data fetching failed", error: error.message });
  }
};
...
export const getPendingUsers = async (req, res) => {
  try {
    ...
    res
      .status(404)
      .json({
        message: "Pending Users Data fetching failed",
        error: error.message,
      });
  }
};
...
export const getActiveUsers = async (req, res) => {
  try {
    ...
    res
      .status(404)
      .json({
        message: "Active Users Data fetching failed",
        error: error.message,
      });
  }
};
...
export const getRejectedUsers = async (req, res) => {
  try {
    ...
    res
      .status(404)
      .json({
        message: "Rejected Users Data fetching failed",
        error: error.message,
      });
  }
};
...
export const revokeUserRole = async (req, res) => {
  try {
    ...
    if (!userId) {
      return res
        .status(400)
        .json({ message: "User ID is required to revoke a role." });
    }
